...
Отвечай на русском. Пиши как короткий Telegram-дайджест: plain text, без markdown, без **,
без таблиц, без ###. Будь конкретным — указывай отправителя и суть.""",
)
...
def run():
    ...
    prompt = f"""Составь короткий Telegram-дайджест входящих писем в plain text.
Не используй markdown, **жирный**,
таблицы, ###, подчёркивания и code blocks.
